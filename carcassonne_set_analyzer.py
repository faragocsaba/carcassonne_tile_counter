...
def find_optimal_grid_parameters(tile_mask):
    """Optimizes grid phase x0, y0 and pitch S for tile mask."""
    h, w = tile_mask.shape
    ...
    for S in range(80, 140):
        ...
        for x0 in range(S):
            ...
            if score_x > best_x_score: best_x_score = score_x; best_x_offset = x0
            
        best_y_score, best_y_offset = -1, 0
        for y0 in range(S):
            score_y = np.sum(h_proj[np.arange(y0, h, S)])
            if score_y > best_y_score: best_y_score = score_y; best_y_offset = y0
